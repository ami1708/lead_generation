...
const CRM_OPTIONS = [
  { value: "hubspot",    label: "HubSpot",    field: "Access Token" },
  { value: "pipedrive",  label: "Pipedrive",  field: "API Token" },
  { value: "zoho",       label: "Zoho CRM",   field: "OAuth Access Token" },
  { value: "salesforce", label: "Salesforce", field: "Access Token", needsUrl: true },
  { value: "teamgate",   label: "Teamgate",   field: "API Key" },
];
...
function leadsToCSV(leads: Lead[]): string {
  const headers = "name,company,contact,interest,lastContact,reason,budget,notes";
  const rows = leads.map((l) =>
    [l.name, l.company || "", l.contact, l.interest, l.lastContact, l.reason, l.budget || "", l.notes || ""].join(",")
  );
...
function ScoreCircle({ score, size = 36 }: { score: number; size?: number }) {
  ...
  return (
    <span className="inline-flex items-center justify-center rounded-full font-black flex-shrink-0"
...
export default function Dashboard() {
  ...
  const selectedCrm = CRM_OPTIONS.find((c) => c.value === crm)!;

  async function fetchCRM() {
    if (!apiKey) { setCrmErr("API key is required"); return; }
    setCrmLoading(true); setCrmErr("");
    try {
      const res = await fetch("/api/crm", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ crm, apiKey, instanceUrl }),
      });
      const d = await res.json();
      if (!res.ok) throw new Error(d.error || "Failed to connect");
      setCsv(leadsToCSV(d.leads));
      setTab("csv");
    } catch (e) {
      setCrmErr(e instanceof Error ? e.message : "Connection failed");
    } finally {
      setCrmLoading(false);
    }
  }
  ...
  async function run() {
    ...
    setErr(""); setProcessing(true); setProgress(10);
    const leads: Lead[] = rows.map((r) => ({
      name: r.name || "", company: r.company || "", contact: r.contact || r.email || r.phone || "",
      interest: r.interest || "", lastContact: r.lastContact || "", reason: r.reason || "",
    ...
    try {
      ...
      clearInterval(ticker); setProgress(98);
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || "Processing failed");
      setResults(d.leads); setSel(d.leads[0] || null); setProgress(100);
    } catch (e) {
  ...
  function exportCSV() {
    const rows = results.map((l) => [l.score, l.name, l.company || "", l.contact, l.interest,
      `"${(l.scoreReason || "").replace(/"/g, "'")}"`, l.lastContact, l.budget || ""]);
  ...
  const Header = ({ extra }: { extra?: React.ReactNode }) => (
    <header className="sticky top-0 z-40 flex items-center justify-between px-8 py-4"
      ...
      <div className="flex items-center gap-3">
        {extra}
        {session?.user && (
          <div className="flex items-center gap-3 pl-3" style={{ borderLeft: border }}>
            {session.user.image && (
              <img src={session.user.image} alt="" className="w-7 h-7 rounded-full" />
            )}
            <span className="text-zinc-400 text-xs hidden sm:block">{session.user.name || session.user.email}</span>
            <button onClick={() => signOut({ callbackUrl: "/" })} className="btn-ghost text-xs px-3 py-1.5">Sign out</button>
          </div>
        )}
      </div>
    </header>
  ...
  if (processing) {
    const steps = [
      ...
      { label: "Scoring each lead with AI", done: progress > 75 },
              ...
              {steps.map((s) => (
                <div key={s.label} className="flex items-center gap-3">
                  <span className="w-4 h-4 rounded-full flex-shrink-0 flex items-center justify-center text-xs transition-all"
                    style={{ background: s.done ? "rgba(0,229,160,0.15)" : "rgba(255,255,255,0.05)", border: s.done ? "1px solid rgba(0,229,160,0.4)" : border, color: s.done ? "var(--teal)" : "transparent" }}>✓</span>
  ...
  if (results.length > 0) {
    ...
    const filtered = results.filter((l) =>
      activeFilter === "hot" ? l.score >= 8 : activeFilter === "warm" ? l.score >= 5 && l.score < 8 : activeFilter === "cold" ? l.score < 5 : true
    );
    return (
      <div className="min-h-screen bg-[#080808] text-white flex flex-col">
        <Header extra={
          <>
            ...
            <button onClick={() => { setResults([]); setSel(null); setActiveFilter("all"); setCsv(DEMO_CSV); }} className="btn-teal text-xs px-4 py-2">New batch</button>
          </>
        } />
        <div className="max-w-6xl mx-auto w-full px-8 py-8 space-y-6">
          <div className="grid grid-cols-5 gap-4">
            {[
              ...
              <button key={s.label} onClick={() => setActiveFilter(s.filter)} className="card p-5 text-left transition-all"
          ...
          <div className="grid lg:grid-cols-5 gap-6">
            <div className="lg:col-span-2">
              <div className="flex items-center justify-between mb-3">
                ...
                {activeFilter !== "all" && <button onClick={() => setActiveFilter("all")} className="text-zinc-600 text-xs hover:text-zinc-400">Show all</button>}
              </div>
              <div className="space-y-1">
                {filtered.map((lead, i) => (
                  <button key={i} onClick={() => setSel(lead)} className="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl transition-all"
                    style={{ background: sel?.contact === lead.contact ? "rgba(255,255,255,0.07)" : "transparent", border: sel?.contact === lead.contact ? "1px solid rgba(255,255,255,0.12)" : "1px solid transparent" }}>
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      {lead.signals && lead.signals.scoreBoost > 0 && <span className="text-xs font-mono font-bold" style={{ color: "var(--teal)" }}>+{lead.signals.scoreBoost}</span>}
              ...
              <div className="lg:col-span-3 space-y-4">
                    ...
                    <div className="space-y-3 mt-4">
                      ...
                      {sel.roiEstimate !== undefined && sel.roiEstimate > 0 && (
                        <div className="rounded-xl px-5 py-4" style={{ background: "rgba(0,229,160,0.05)", border: "1px solid rgba(0,229,160,0.18)" }}>
                          ...
                          {sel.roiReason && <p className="text-zinc-400 text-xs mt-2 leading-relaxed">{sel.roiReason}</p>}
                        </div>
                      )}
                    </div>
                  )}
                ...
                {sel.signals && sel.signals.scoreBoost > 0 && (
                  <div className="card overflow-hidden" style={{ borderColor: "rgba(168,85,247,0.25)" }}>
                    <div className="px-5 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(168,85,247,0.12)", background: "rgba(168,85,247,0.06)" }}>
                      <div>
                        <p className="label" style={{ color: "#c084fc" }}>Live web signals</p>
                        <p className="text-zinc-600 text-xs mt-0.5">Found by Tavily search</p>
                      </div>
                      <span className="text-sm font-black px-3 py-1 rounded-full" style={{ background: "rgba(168,85,247,0.12)", color: "#c084fc" }}>+{sel.signals.scoreBoost} boost</span>
                    </div>
    ...
    <div className="min-h-screen bg-[#080808] text-white flex flex-col">
      ...
      <div className="flex-1 flex items-start justify-center px-8 py-12">
        <div className="w-full max-w-xl space-y-6">
          ...
          <div>
            <h1 className="font-serif text-3xl font-black mb-1">Score your leads</h1>
            <p className="text-zinc-500 text-sm">Upload a CSV or connect your CRM — AI scores every lead in minutes.</p>
          </div>

          {/* Tabs */}
          <div className="flex gap-1 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.04)", border }}>
            {[{ id: "csv", label: "Upload CSV" }, { id: "crm", label: "Connect CRM" }].map((t) => (
              <button key={t.id} onClick={() => setTab(t.id as "csv" | "crm")}
                className="flex-1 py-2 text-sm font-semibold rounded-lg transition-all"
                style={tab === t.id ? { background: "rgba(255,255,255,0.08)", color: "#fff" } : { color: "#71717a" }}>
                {t.label}
              </button>
            ))}
          </div>
          ...
          {tab === "csv" ? (
            <>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={() => setCsv(DEMO_CSV)} className="text-zinc-600 text-xs hover:text-zinc-400 transition-colors">Reset demo</button>
                </div>
              ...
              {err && <div className="px-4 py-3 rounded-xl text-sm text-red-400" style={{ background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.2)" }}>{err}</div>}
              <button onClick={run} disabled={!csv.trim()} className="btn-teal w-full text-sm font-semibold teal-glow" style={{ height: "52px" }}>
                Score leads with AI →
              </button>
            </>
          ) : (
            <>
              <div className="space-y-3">
                <div>
                  <p className="label mb-2">CRM platform</p>
                  <div className="grid grid-cols-3 gap-2">
                    {CRM_OPTIONS.map((c) => (
                      <button key={c.value} onClick={() => { setCrm(c.value); setCrmErr(""); }}
                        className="py-2.5 px-3 rounded-xl text-sm font-medium transition-all"
                        style={crm === c.value
                          ? { background: "rgba(0,229,160,0.1)", border: "1px solid rgba(0,229,160,0.3)", color: "var(--teal)" }
                          : { background: "rgba(255,255,255,0.03)", border, color: "#a1a1aa" }}>
                        {c.label}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  <p className="label mb-2">{selectedCrm.field}</p>
                  <input className="inp w-full" placeholder={`Paste your ${selectedCrm.field.toLowerCase()} here`}
                    value={apiKey} onChange={(e) => setApiKey(e.target.value)} type="password" />
                </div>

                {selectedCrm.needsUrl && (
                  <div>
                    <p className="label mb-2">Instance URL</p>
                    <input className="inp w-full" placeholder="https://yourorg.salesforce.com"
                      value={instanceUrl} onChange={(e) => setInstanceUrl(e.target.value)} />
                  </div>
                )}

                <p className="text-zinc-700 text-xs">Your credentials are used only to fetch leads and are never stored.</p>

                {crmErr && <div className="px-4 py-3 rounded-xl text-sm text-red-400" style={{ background: "rgba(248,113,113,0.06)", border: "1px solid rgba(248,113,113,0.2)" }}>{crmErr}</div>}

                <button onClick={fetchCRM} disabled={crmLoading || !apiKey}
                  className="btn-teal w-full text-sm font-semibold teal-glow" style={{ height: "52px" }}>
                  {crmLoading ? "Connecting…" : `Connect ${selectedCrm.label} & import leads →`}
                </button>
              </div>
            </>
          )}
          ...
          <p className="text-zinc-700 text-xs text-center">
            Tavily searches the web per lead · AI scores 1–10 · Results ranked by revival likelihood
          </p>
        </div>
      </div>
    </div>
